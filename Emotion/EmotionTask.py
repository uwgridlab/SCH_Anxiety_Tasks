...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/llevinson/Google Drive UW/SCH_Anxiety_Tasks/Emotion/EmotionTask.py',
...
for thisBlock in blocks:
    ...
    for thisTrial in trials:
        ...
        thisExp.addData('ImageFile', df_sm.ImageFile[loc_sub[trials.thisRepN]]);
        thisExp.addData('Category', df_sm.Category[loc_sub[trials.thisRepN]]);
        thisExp.addData('Num', df_sm.Num[loc_sub[trials.thisRepN]]);
    ...
    # keep track of which components have finished
    block_completedComponents = [completed_text]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if completed_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > completed_text.tStartRefresh + 1-frameTolerance:
                # keep track of stop time/frame for later
                completed_text.tStop = t  # not accounting for scr refresh
                completed_text.frameNStop = frameN  # exact frame index
                win.timeOnFlip(completed_text, 'tStopRefresh')  # time at next scr refresh
                completed_text.setAutoDraw(False)
